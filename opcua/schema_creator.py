...
async def main():
    ...
    var_1 = await server.nodes.objects.add_variable(f"ns={qi_namespace},i=1002",
                                                    "StartInference",
                                                    True,
    ...
    node_list = [dev, mydevice[0], myobj, myvar, var_1, var_2]

    exporter = XmlExporter(server)
    await exporter.build_etree(node_list)
    await exporter.write_xml('./opcua/schemas/ua-export.xml')
